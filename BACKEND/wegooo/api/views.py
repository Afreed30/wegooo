...
def book_seats(request):
    try:
        ...
        if not schedule_id or not selected_seats:
            return Response({"error": "Missing schedule_id or selected_seats"}, status=400)

        seats = Seat.objects.filter(id__in=selected_seats, schedule_id=schedule_id, is_available=True)
        ...
        return Response({"message": f"✅ {len(selected_seats)} seat(s) booked successfully!"}, status=201)
...
# ====================================================
# ✅ 6️⃣ Razorpay + Booking Integration (New)
# ====================================================
@api_view(['POST'])
@transaction.atomic
def book_seats(request):
    try:
        data = request.data
        schedule_id = data.get("schedule_id")
        seat_numbers = data.get("seat_numbers", [])  # ✅ Correct field name
        username = data.get("username")
        ...
        # ✅ Get seats by seat_number (not id)
        ...
        # ✅ Create bookings and mark seats unavailable
        for seat in seats:
            Booking.objects.create(
                user_id=user.id,
                schedule_id=schedule_id,
